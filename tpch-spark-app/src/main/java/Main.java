...
public class Main {
    ...
    private static void writeTheParquetData(SparkSession spark, String tblReadPath,String writeParquetPath){
        ...
        spark.read().format("csv").schema(customer_schema).option("delimiter", "|").csv(/*tblReadPath + "customer.tbl"*/ "file:///mnt1/sparkauto/generatedData/tpch-spark-master/dbgen/customer").write().mode("overwrite").parquet(writeParquetPath+"customer");
        //done
